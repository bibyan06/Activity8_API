...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action']) && $_GET['action'] === 'getUsers') {
    $stmt = $pdo->query("
        ...
        FROM 
            accounts a
        JOIN 
            department d ON a.dname = d.dnumber
        JOIN 
            deptsal s ON d.dnumber = s.dnumber
    ");
    $users = $stmt->fetchAll();
    ...
    $sql = "INSERT INTO accounts (username, pass, email, dname, totalsalary) VALUES (?, ?, ?, ?, ?)";
    $stmt = $pdo->prepare($sql);
    $stmt->execute([$input['username'], $input['pass'], $input['email'], $input['dname'], $input['totalsalary']]);
